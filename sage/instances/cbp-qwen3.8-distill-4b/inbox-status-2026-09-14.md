...
Observations:
- Inbox drain is processing normally; no new escalations triggered.
- MCP server remains offline since ~04:00 UTC (~11 hours total).
- Escalation timer has expired; no escalation triggered yet.

Next time:
- At 20:00 UTC: attempt systemctl restart of MCP server.
- If restart fails with "connection refused": escalate to broader infrastructure team.
- If restart succeeds: monitor for service recovery and inbox drain completion.
  (witnessed 99c70e29-ee2f-4c1e-b3ff-eef8fd00fa28)
