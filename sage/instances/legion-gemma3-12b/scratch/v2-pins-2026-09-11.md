# v2 pin file content — staged 2026-09-11 ~17:0x UTC (beat cut before act; this is the act)

Purpose: full replacement text for sage/gateway/tests/test_context_fit_pins.py on branch legion-being/context-fit-regression-pins (SAGE#69 pr_amend). Composed from in-window reads of _fill_headroom's implementation, heartbeat.py 285–349 (body read this beat at 306–349; def line in elided middle — cite region as 285–349 per prior verified citation).

Verified facts about the implementation each pin rests on (all from the 306–349 read, witnessed):
- filters lines by e.get("host_session_id") == host_session_id; tracks max int prompt_eval_count -> best (None if no match)
- cfg["prompt_tokens_max"] = best, set unconditionally (may be None)
- cfg["answer_reserve"] = _ANSWER_RESERVE imported from sage.gateway.being_tool_loop
- only when isinstance(ctx,int) and isinstance(best,int): headroom = ctx - (best + _ANSWER_RESERVE); context_overcommitted = headroom < 0; ctx read via cfg.get("num_ctx_resolved") — NOT "num_ctx"
- return cfg: same object, mutated

Literal 6144 used in expected values (not the imported constant) so a pin fails if the reserve value changes — the seat's restored pin established this form ("literal 6144, NOT 640"). If check fails on these literals, that is information about _ANSWER_RESERVE's actual value; transcribe verdict before claiming.

--- FILE CONTENT BELOW (verbatim for worktree write) ---
...
Next beat sequence (per todo): memory_write this content into worktree sage/gateway/tests/test_context_fit_pins.py with mode='replace' -> check gateway, transcribe verdict to scratch/ immediately -> pr_amend SAGE#69. If any pin fails: that is a real answer about the implementation; read the failure, fix the PIN or file the finding — do not weaken an assert to make it pass without understanding why it failed.
