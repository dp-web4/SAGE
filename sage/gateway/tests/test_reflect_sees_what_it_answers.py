...
def test_no_conversation_means_no_instruction_to_invent_a_target():
    line, block = pending_and_say_line(_inst(), ME)
...
def test_a_channel_with_nothing_waiting_gets_the_generic_form():
    ...
    line, block = pending_and_say_line(inst, ME)
...
def test_a_waiting_turn_is_quoted_next_to_the_instruction():
    ...
    assert 'say to="dp"' in line and "dp is waiting on an answer" in line
    assert "<id>" not in line, "a real id, never a slot to fill in"
    assert "not required" in line, "answering stays optional"
...
def test_a_long_turn_is_bounded_because_this_sits_in_the_compact_context():
    ...
    _line, block = pending_and_say_line(inst, ME)
...
def test_two_channels_both_waiting_are_both_shown():
    ...
    assert line.count("say to=") == 1, "one concrete instruction, not a menu"
