...
# Warm the embedder now, not on the being's first remember. Measured 2026-09-13: a memory_search
# warm-up (machines/nomad/warm_membot.py) does NOT load the model when the cartridge is empty, so the
# first remember paid the 12 s load and timed out. /api/embed always embeds and mounts nothing.
for i in $(seq 1 60); do curl -s -m 2 -o /dev/null "http://127.0.0.1:$PORT/api/status" && break; sleep 1; done
curl -s -m 300 -o /dev/null -w "warm-up embed HTTP %{http_code} in %{time_total}s\n" "http://127.0.0.1:$PORT/api/embed" \
    -H "Content-Type: application/json" -d '{"texts":["warm"]}' >> "$LOG" 2>&1 || true
pid=$(lsof -t -i :$PORT 2>/dev/null | head -1)
